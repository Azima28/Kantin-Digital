...
final adminDashboardProvider =
  ...
  try {
    ...
    for (var row in studentRes) {
      studentBalanceSum +=
          double.tryParse(row['balance'].toString()) ?? 0.0;
    }

    // 2. Fetch Merchant Balance
    final merchantRes =
        await client.from('canteen_operators').select('balance_earned');
...
final adminSettingsProvider =
  ...
  final List<dynamic> res =
      await client.from('system_settings').select('key, value');
...
final adminStudentDetailProvider =
    FutureProvider.autoDispose.family<AdminStudentDetail, String>(
        (ref, id) async {
  final client = ref.read(supabaseClientProvider);
  ...
  // 2. Fetch student
  final student =
      await client.from('students').select().eq('id', id).single();
...
final adminParentDetailProvider =
    FutureProvider.autoDispose.family<AdminParentDetail, String>(
        (ref, id) async {
  final client = ref.read(supabaseClientProvider);
...
final adminMerchantDetailProvider =
    FutureProvider.autoDispose.family<AdminMerchantDetail, String>(
        (ref, id) async {
  final client = ref.read(supabaseClientProvider);
  ...
  final operator =
      await client.from('canteen_operators').select().eq('id', id).single();
...
final adminFinanceDetailProvider =
    FutureProvider.autoDispose.family<AdminFinanceDetail, String>(
        (ref, id) async {
  final client = ref.read(supabaseClientProvider);
  ...
  final officer =
      await client.from('finance_officers').select().eq('id', id).single();
